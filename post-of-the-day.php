...
function potd_get_random_post( $curr_post, $prev_post ) {
	...
	if ( ( ! empty( $potd_categories ) ) || ( ! empty( $potd_post_types ) ) ) {
		if ( ( ! empty( $potd_categories ) ) && ( empty( $potd_post_types ) ) ) { // categories no post_types
		...
		elseif ( ( empty( $potd_categories ) ) && ( ! empty( $potd_post_types ) ) ) { // post_types no categories
		...
		elseif ( ( ! empty( $potd_categories ) ) && ( ! empty( $potd_post_types ) ) ) { // post_types and categories
			foreach ( $potd_post_types as $potd_post_type ) {
				foreach ( $potd_categories as $potd_category ) {
					$args                    = array(
						'numberposts'    => -1,
